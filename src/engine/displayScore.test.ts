 ...
 * - パッシブ欄: −1.8〜+0.8(供給側の発動確率で重みづけした仮説。ケース A の −1.3 は赤スコアサポートの配賦が未解明のぶん。
 *   9.8〜9.11 の −0.7〜−1.8 は恒常マリン 1凸のパッシブ(3期生 2 人のスコアサポート、ぺこらとの 2 人で成立)を含む形成で、
 *   2026-09-12 に 1凸の値を仮定倍率の 10.9 から抽出マスターの level 1 = 9% に訂正したことで 0.2 ずつ広がった。実機 2.4〜3.5 に対し
 *   モデル 1.7〜2.1 で、正しい入力ほど不足が大きい — パッシブ支援の表示換算(pending.md「5カテゴリの内部式」)が未解明のぶん)
...
describe("表示スコアボーナスのゴールデンケース(2026-09-08 実機 20 ケース)", () => {
  ...
  it("20 ケースの実機との誤差が現在の範囲に収まる(悪化の検知。この値に合わせ込むためのテストではない)", () => {
    ...
    // アクティブ 0(完全一致) / ボード 2.7 / パッシブ 1.8 / SP 0.6 / 合計 2.3 / ユニットスコア 0.99%。上限はそれをわずかに上回る値で、
    // 下げる(精度を上げる)ときはこの上限も下げる(ボードは 2026-09-11 に 3.0 → 2.8 へ締めた)。
    // 上限は緩めない — 緩めるのは入力の実機値・カードデータの訂正のときだけで、モデルの変更を通すためには緩めない。
    // パッシブは 2026-09-12 に恒常マリン 1凸のパッシブを仮定倍率 10.9 → 抽出マスター level 1 = 9% に訂正したことで 1.6 → 1.8(9.10)へ
    // 広がった(モデル不変・入力訂正。カードデータは src/data/bloomMasterVariants.test.ts で固定)
    const limits = {
      active: 0.001,
      board: 2.8,
      passive: 1.85,
