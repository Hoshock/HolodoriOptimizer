...
const cases: GoldenCase[] = [
  ...
  },
  {
    ...
    // 15.0% は当時のカード表示の記録。基準を 39 に訂正した後もそのまま置く(21 に読み替える根拠がない — pending.md「5カテゴリの内部式」「SP残差」)
...
describe("表示スコアボーナスのゴールデンケース(2026-09-08 実機 20 ケース)", () => {
  ...
  it("SP 欄は 20 ケース中 15 が完全一致し、外れるのは発動率 UP を含む形成だけ(±0.6)", () => {
    const mismatched = golden
      ...
      .map(([name]) => name);
    // 9.7 / 9.8 / 9.10 / 9.11 / 9.12 は SP に「スキル発動率 +X%」があり、その項の式が未解明(pending.md「5カテゴリの内部式」「SP残差」)
...
describe("赤スコアサポート +10 の 水着ノエル 0凸 入替(2026-09-11 実機観測。固定 0.88 × X を棄却した対照)", () => {
   ...
   * SP 欄は −1.3(ノエル 0凸の SP 100% × 12 秒。発動率 UP は 3期生 1 人で不発 — SP の式の未解明部分、pending.md「5カテゴリの内部式」「SP残差」)。
