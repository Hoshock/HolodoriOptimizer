 ...
 * 観測値の全文は docs/human/repro/display-score-20260908-11.md「2026-09-08: 同じメンバー 5 枚でリーダー 2 通り」)。
 *
 ...
 * 実機とのずれは「実装した式 − 実機」で、項目ごとの傾向(2026-09-13 に表示 5 欄を
 * 「source ごとの増分の和 × source ごとの raw weight での按分」(`attributeDisplaySupport`)へ載せ替えた後の数字):
 * - アクティブ欄: **20 ケースすべて実機と完全一致**(5 人共通タイムライン・確率 55/46/37 + permil 切り上げ)
 * - SP 欄: **20 ケースすべて実機と完全一致**(表示アクティブ欄 × Σ 効果時間 × 支援/12000 × (1 + 発動率 UP/200) の
 *   permil 切り上げ。2026-09-13 に発動率 UP の閉じた形が決まるまでは、発動率 UP を含む 5 件が −0.4〜+0.6 外れていた —
 *   displayScoreSpecialColumn.test.ts)
 * - ホロメンボード効果欄: −0.5〜+0.1(載せ替え前は −1.5〜+2.7)。青の発動率は乗算型 `min(1, p0 × (1 + r/100))`、
 *   頻度は周期 ÷ (1 + f)。**頻度の山(9.13 < 9.14 > 9.15)はモデルも再現する**(4.3 < 4.6 > 4.5。12% まで含む系列は
 *   下の「発動頻度系列」)。
 *   9.16 / 9.17 は発動率・頻度だけを変えた実験ではない — 同時に静的な T/S/P のマスも外している(下の note とテスト名、
 *   docs/human/repro/display-score-20260908-11.md の 9.16 / 9.17 を参照)。ボードは経路がつながっていないマスを単独で外せないので、
 *   「発動率だけを変えた 2 点」は実機では作れない(.claude/rules/game-facts.md「実験」の設計ルール)
 * - パッシブ欄: 0.0〜+0.2、20 ケース中 9 が完全一致(載せ替え前は −1.8〜+0.8)。パッシブのスコアサポートは**静的**
 *   (供給側の発動確率で割り引かない。gated 型は K6 で棄却 — docs/human/display-score.md)
 * - 合計: −0.5〜+0.5 pt、ユニットスコアは −0.22%〜+0.23%(載せ替え前は −0.8〜+2.3 pt / −0.4%〜+1.0%)
 * - ケース A(リーダーの赤「全員のスコアサポート効果 +28.1%」あり)の赤の増分は、2026-09-11〜12 の候補秒率式
 *   `X × 基準候補秒 / T` を経て、2026-09-13 に総量 `(X/100) × E_blue` + ボード欄の raw weight `W_B` への寄与へ
 *   置き換わった(候補秒率式は一般式として反証され実装ごと削除 — ADR-006 の 2026-09-13 Update)。
 * 誤差の幅は 2026-09-09 の青ボードの実効値の訂正(フブキ 39・ぺこら 30)で一度広がった — モデルを変えたのではなく
 * **入力の実機値が変わった**ため。
 * 総合力は実機値をそのまま与える(総合力側の ±2 は power.test.ts で別に固定)。
 * 恒常マリン 1凸のパッシブ(3期生 2 人のスコアサポート、開花最大 12%)は 2026-09-12 に抽出マスターの level 1 = 9% を
 * bloomVariants に入れた(実機目視ではなく外部解析由来 — src/data/bloomEvidence.ts)。旧仮定倍率の 12 ÷ 1.1 ≈ 10.9 は使わない。
 */
...
/**
 * 水着フワワ 0凸入りの 3 編成目(2026-09-12)の観測時点の青。この時点の値が未確定なので 2026-09-09 の実機値 45 / 0 を置く
 * (09-12 の構造化データには青マスがなく 0 / 0。2026-09-13 の export の 24 / 0 は振り直し後の別時点なので裁定に使えない — pending.md)
...
const cases: GoldenCase[] = [
  ...
  },
  {
    ...
    // 15.0% は当時のカード表示の記録。基準を 39 に訂正した後もそのまま置く(21 に読み替える根拠がない —
    // pending.md「2026-09-12表示スコアGoldenの入力条件再確認」)
...
const golden: [string, Row, Row][] = [
  ["B", [77.0, 13.2, 2.2, 46.0, 138.4, 1245189], [77, 12.8, 2.3, 46, 138.1, 1243623]],
  ["A", [77.0, 36.7, 3.1, 46.0, 162.8, 1660900], [77, 36.3, 3.2, 46, 162.5, 1659004]],
  ["ok f8", [77.0, 10.9, 1.9, 46.0, 135.8, 1231609], [77, 10.5, 2, 46, 135.5, 1230042]],
  ["ok f4", [77.0, 8.0, 1.5, 46.0, 132.5, 1214373], [77, 7.6, 1.5, 46, 132.1, 1212284]],
  ["ok f0", [77.0, 5.7, 1.1, 46.0, 129.8, 1200271], [77, 5.3, 1.1, 46, 129.4, 1198182]],
  ["ok r32", [77.0, 10.8, 1.9, 46.0, 135.7, 1231087], [77, 10.4, 2, 46, 135.4, 1229520]],
  ["ok r25", [77.0, 10.5, 2.0, 46.0, 135.5, 1227581], [77, 10.1, 2, 46, 135.1, 1225496]],
  ...
  ["9.7", [76.7, 10.0, 1.1, 44.2, 132.0, 911375], [76.7, 9.9, 1.2, 44.2, 132, 911375]],
  ["9.8", [63.7, 13.9, 3.0, 38.6, 119.2, 1106433], [63.7, 13.7, 3.1, 38.6, 119.1, 1105928]],
  ["9.9", [68.8, 12.1, 2.9, 41.7, 125.5, 1148188], [68.8, 12.1, 3, 41.7, 125.6, 1148697]],
  ["9.10", [70.4, 14.3, 3.5, 42.1, 130.3, 1133037], [70.4, 14.2, 3.6, 42.1, 130.3, 1133037]],
  ["9.11", [75.6, 12.5, 2.4, 45.2, 135.7, 1191293], [75.6, 12.4, 2.5, 45.2, 135.7, 1191293]],
  ["9.12", [78.4, 4.5, 0.9, 44.0, 127.8, 1100085], [78.4, 4.6, 1, 44, 128, 1101051]],
 ...
 * displayScoreSpecialColumn.test.ts「未確認を含む編成」）。K6 のパッシブ欄 2.9 は
...
describe("表示スコアボーナスのゴールデンケース(2026-09-08 実機 20 ケース)", () => {
  ...
  it("SP 欄は 20 ケースすべて実機と完全一致する(発動率 UP の閉じた形が決まった 2026-09-13 以降)", () => {
    const mismatched = golden
      .filter(([, observed, model]) => model[3] !== observed[3])
      .map(([name]) => name);
    // 以前は 9.7 / 9.8 / 9.10 / 9.11 / 9.12(SP に「スキル発動率 +X%」がある形成)だけ最大 0.6 外れていた。
    // 発動率 UP が「表示アクティブ欄 × 効果時間/120 × 支援/100 × (1 + X/200)」だと決まって 0 になった
    // (displayScoreSpecialColumn.test.ts)
  ...
  it("20 ケースの実機との誤差が現在の範囲に収まる(悪化の検知。この値に合わせ込むためのテストではない)", () => {
    // 現状の最大誤差(2026-09-15 にカードデータを実機値へ直した後の回帰値):
    // アクティブ 0(完全一致) / ボード 0.4 / パッシブ 0.1 / SP 0(完全一致) / 合計 0.4 / ユニットスコア 0.17%。
    // 2026-09-13 に SP 欄を閉じた形へ替えた時点では ボード 0.5 / パッシブ 0.2 / 合計 0.5 / 0.22%、
    // その載せ替え前は ボード 2.7 / パッシブ 1.8 / 合計 2.3 / 0.99% だった。
    // 上限はそれをわずかに上回る値で、下げる(精度を上げる)ときはこの上限も下げる。
    // 上限は緩めない — 緩めるのは入力の実機値・カードデータの訂正のときだけで、モデルの変更を通すためには緩めない。
    const limits = {
      active: 0.001,
      board: 0.45,
      passive: 0.15,
 ...
 * 実機のボード欄は 4.6(0) → 4.7(4) → 4.4(8) → 5.0(12) と単調でない。2026-09-13 の projective 配分では
 * モデルもこの非単調性を再現する(4.5 → 4.6 → 4.3 → 4.9、4 点とも 0.1 以内) — 差分配賦だった頃は
 * 5.2 → 5.9 → 6.1 → 7.1 と単調増加で説明できなかった。残る未確定は青の重みの正規化 — pending.md「W_blue の発動頻度側の正規化」
 */
/**
...
/**
 * 黄ボードの楽曲スコアボーナスの適用位置(2026-09-11 ユーザー実機観測。観測値の全文は docs/human/repro/display-score-20260908-11.md「2026-09-11: 黄ボードの適用位置」)。
 *
...
/**
 * 赤の歌唱者条件と黄 10% の 4 象限(2026-09-11 ユーザー実機観測。観測値の全文は docs/human/repro/display-score-20260908-11.md「2026-09-11: 赤の歌唱者条件」)。
 *
 ...
 *   モデルの増分は ボード +20.5・パッシブ +0.3・合計 +20.8(2026-09-13 の projective 配分。2026-09-11〜12 に使っていた
 *   候補秒率式 24 × 175/200 = 21.0 は一般式として反証され、実装ごと削除済み)
 *
...
describe("赤の歌唱者条件と黄の 4 象限(2026-09-11 実機観測)", () => {
  ...
  const modelRows: Row[] = [
    [77, 36.3, 3.2, 46, 162.5, 1652319],
    [77, 56.9, 3.5, 46, 183.4, 1856729],
    [77, 58.9, 3.2, 46, 185.1, 1794575],
    [77, 79.5, 3.5, 46, 206, 2004795],
  ];
...
/**
 * 水着おかゆリーダーの 3 点(2026-09-11 ユーザー実機観測。観測値の全文は docs/human/repro/display-score-20260908-11.md「2026-09-11: 赤 R-001 / R-002 の単独差分」)。
 * この 5 人の赤 +10 → 合計 +8.8 は、同日に一度「0.88 × X の実機再現則」と置かれたが、メンバー 1 人を替えた 2 編成目
 * (下の 水着ノエル 0凸 入替)で +8.5 になり、編成非依存の固定係数は棄却された。ここでは実機の 4 点の関係と、
 * 現在のモデル(総量 `(X/100) × E_blue` + projective 配分)の値を固定する。
 *
...
describe("水着おかゆリーダーの 3 点(2026-09-11 実機観測。赤 +10 → 合計 +8.8 の旧編成)", () => {
  ...
  const modelRows: Row[] = [
    [77, 12.8, 2.3, 46, 138.1, 1252233],
    [77, 12.8, 2.3, 46, 138.1, 1255983],
    [77, 19.6, 2.3, 46, 144.9, 1291853],
  ...
  it("実カードでも R-002 OFF → ON(赤 +10)の増分は ボード +8.3 / パッシブ +0.4 / 合計 +8.7(実機 8.4 / 0.4 / 8.8)。黄 3.0% は +6.7(実機 6.8)", () => {
    ...
    expect(round1(p3.board - p2.board)).toBe(8.2);
    expect(round1(p3.passive - p2.passive)).toBe(0.5);
    ...
    expect(round1(p2.board - p1.board)).toBe(6.8);
...
/**
 * 赤「全員のスコアサポート効果 +X%」の 水着ノエル 0凸 入替(2026-09-11 ユーザー実機観測、恒常ぺこら → 水着ノエル 0凸。
 * 観測値の全文は docs/human/repro/display-score-20260908-11.md「恒常ぺこら → 水着ノエル 0凸 の入替」)。
 *
 ...
 * 経緯: 2026-09-11〜12 はここに raw 増分 = X × 基準候補秒 / 200 を置いていた(旧編成 175/200 → 8.75、
 * ノエル編成 168/200 → 8.4)が、その後の追加実測で一般式として反証され、2026-09-13 に実装ごと削除した
 * (ADR-006 の 2026-09-13 Update)。現在は総量 `(X/100) × E_blue` + projective 配分で、この編成は 3 欄とも実機と完全一致する。
 *
...
describe("赤スコアサポート +10 の 水着ノエル 0凸 入替(2026-09-11 実機観測。固定 0.88 × X を棄却した対照)", () => {
   ...
   * SP 欄は実機と完全一致(ノエル 0凸の SP 100% × 12 秒。発動率 UP 40% は 3期生 1 人で不発)。
   * 赤 +10 の増分はモデルも ボード +8.0 / パッシブ +0.5 / 合計 +8.5 で、実機と 3 欄とも完全一致する
   */
  const modelRows: Row[] = [
    [78.9, 17.1, 1.6, 46.3, 143.9, 1278667],
    [78.9, 25.2, 2.1, 46.3, 152.5, 1323753],
  ];
  ...
  it("実カードでも R-002 OFF → ON(赤 +10)の増分はモデルで ボード +8.0 / パッシブ +0.5 / 合計 +8.5 — 実機と 3 欄とも完全一致", () => {
    ...
    expect(round1(on.total - off.total)).toBe(8.6);
    expect(round1(on.board - off.board)).toBe(8.1);
...
/**
 * 赤スコアサポート +10 の 3 編成目(2026-09-12 ユーザー実機観測、水着フワワ 0凸入り。観測値の全文は
 * docs/human/repro/display-score-20260912.md「旧R-002 +10単独差分」)。
 *
 ...
 *
 * 経緯: 2026-09-12 の時点では、当時採用していた候補秒率式 X × 基準候補秒 / T(この 5 人は 174/200 → 10 × 0.87 = 8.7)が
 * **3つの異なるメンバー編成**で実機と一致していた。その後の追加実測で一般式として反証され、2026-09-13 に実装ごと削除した。
 * 棄却できたのは「表示合計 = 0.88 × X」のような編成非依存の固定**表示**係数(0.88 なら raw 8.8 で
 * ノエル編成の +8.5 と 0.3 差 — ボード / パッシブ 2 欄の独立量子化(合わせて ±0.2 未満)では説明できない)で、
 * raw = c × X(c ≈ 0.86〜0.87)のような別の編成非依存の raw 固定係数をこの 3 点だけで数学的に排除したわけではない。
 *
 ...
 * 「当時の候補秒率式を反証した」と読まれかけた。カード ID 単位で cardAtBloom から取り直した正しい値が上の 174 で、通常の
 * アクティブ欄 77.9 が実機と完全一致することが入力の独立した検証になる(赤の結果に合わせてカード値を選んだのではない)。
 * このテストのフィクスチャは手打ちの周期 / 効果時間ではなく実カード ID + cardAtBloom から取る。
 *
 * 青の実効値はノエル編成と同じ当時の構造化データの値、フワワだけは観測時点が未確定なので 2026-09-09 の実機値 45 / 0
...
describe("赤スコアサポート +10 の 3 編成目(2026-09-12 実機観測、水着フワワ 0凸入り。実機 +8.7)", () => {
   ...
   */
  const modelRows: Row[] = [
    [77.9, 18.6, 0.8, 47.2, 144.5, 1304223],
    [77.9, 27.1, 1.1, 47.2, 153.3, 1351165],
  ];
  ...
  it("実カードでも R-002 OFF → ON(赤 +10)の増分はモデルで ボード +8.5 / パッシブ +0.2 / 合計 +8.7(実機 8.4 / 0.3 / 8.7)", () => {
    ...
    expect(round1(on.total - off.total)).toBe(8.8);
    expect(round1(on.board - off.board)).toBe(8.5);
    expect(round1(on.passive - off.passive)).toBe(0.3);
    expect(on.active).toBe(off.active);
    expect(on.special).toBe(off.special);
 ...
 * では合わない。衣装欄は 2026-09-13 の projective 配分(attributeDisplaySupport)で production が返すが、この観測は赤の X と
 * 青の観測時点の状態が数値として共有されていないので、5 欄の絶対値はモデルと突き合わせない(このテストが固定するのは
 * 黄の増分と raw の基底)。残る未確定は配分の重みの正規化 — pending.md「`W_blue` の発動頻度側の正規化」。
 */
