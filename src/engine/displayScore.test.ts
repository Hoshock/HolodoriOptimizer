 ...
 *   スコアサポートだけの形成は全件一致する)
 * - ホロメンボード効果欄: −1.9〜+2.0(青の発動率 +r は確率に加算・頻度は周期 ÷ (1 + f))。頻度の山
 ...
 * - 合計: −1.3〜+2.0 pt、ユニットスコアは −0.6%〜+0.9%(合計の最大誤差は permil 切り上げの前より
 *   0.2 pt 増えたが、それは丸めの偶然の打ち消しが消えたためで、完全一致は 1 件 → 3 件に増えた)
...
/**
 * 実機の青ボードの表示値(発動率 UP %, 発動頻度 UP %)。ホロメン単位でカードに載る。
 * みこ 36 / 0 とフワワ 45 / 0 は 2026-09-09 のユーザー実機確認(それまでは未共有で 0 と仮定していた。
 * みこは同日に 37 → 36 と訂正)。
 * マリン恒常は 1 マスも未解放で 0 / 0(status.md)。ミオは青の発動率・頻度のマスが未解放。
 */
...
const golden: [string, Row, Row][] = [
  ...
  ["9.7", [76.7, 10.0, 1.1, 44.2, 132.0, 911375], [76.7, 11.9, 1.1, 44.3, 134.0, 919232]],
  ["9.8", [63.7, 13.9, 3.0, 38.6, 119.2, 1106433], [63.7, 13.2, 1.9, 39.2, 118.0, 1100375]],
  ["9.9", [68.8, 12.1, 2.9, 41.7, 125.5, 1148188], [68.8, 11.5, 2.3, 41.7, 124.3, 1142078]],
  ["9.10", [70.4, 14.3, 3.5, 42.1, 130.3, 1133037], [70.4, 16.1, 1.9, 41.9, 130.3, 1133037]],
  ["9.11", [75.6, 12.5, 2.4, 45.2, 135.7, 1191293], [75.6, 12.6, 1.9, 44.8, 134.9, 1187249]],
  ["9.12", [78.4, 4.5, 0.9, 44.0, 127.8, 1100085], [78.4, 3.8, 1.3, 43.9, 127.4, 1098153]],
  ["9.13", [77.0, 4.4, 0.5, 42.7, 124.6, 1137414], [77.0, 5.4, 0.4, 42.7, 125.5, 1141972]],
  ["9.14", [77.0, 4.7, 0.5, 42.7, 124.9, 1138934], [77.0, 5.2, 0.4, 42.7, 125.3, 1140959]],
  ["9.15", [77.0, 4.6, 0.5, 42.7, 124.8, 1138427], [77.0, 4.7, 0.4, 42.7, 124.8, 1138427]],
  ["9.16", [77.0, 4.3, 0.5, 42.7, 124.5, 1131410], [77.0, 4.1, 0.4, 42.7, 124.2, 1129898]],
  ["9.17", [77.0, 4.1, 0.5, 42.7, 124.3, 1128236], [77.0, 3.2, 0.4, 42.7, 123.3, 1123206]],
];
...
describe("表示スコアボーナスのゴールデンケース(2026-09-08 実機 20 ケース)", () => {
  ...
  it("20 ケースの実機との誤差が現在の範囲に収まる(悪化の検知。この値に合わせ込むためのテストではない)", () => {
    // 現状の最大誤差(2026-09-09、permil 切り上げ導入後): アクティブ 0(完全一致) / ボード 2.0 /
    // パッシブ 1.6 / SP 0.6 / 合計 2.0 / ユニットスコア 0.87%。上限はそれをわずかに上回る値で、
    // 下げる(精度を上げる)ときはこの上限も下げる
    const limits = {
      active: 0.001,
      board: 2.1,
      passive: 1.7,
...
/**
 * 青ボードの発動頻度だけを変えた 4 点(フブキ水着の発動率 33% 固定。0 / 4 / 8% は 9.15 / 9.14 / 9.13 と同じ状態で、
...
describe("青ボードの発動頻度系列(フブキ水着。実機はボード欄・パッシブ欄のみ)", () => {
  ...
  for (const [frequency, obBoard, obPassive, board, passive] of series) {
    it(`頻度 ${String(frequency)}%: 実機 ボード ${String(obBoard)} / パッシブ ${String(obPassive)} → モデル ${String(board)} / ${String(passive)}`, () => {
      const members = [member(OK1), member(MK), member(FB, [33, frequency]), member(MI), member(P)];
