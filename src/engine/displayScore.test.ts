 ...
 * - 合計: −1.4〜+1.9 pt、ユニットスコアは −0.6%〜+0.9%
 ...
 * みこ 37 / 0 とフワワ 45 / 0 は 2026-09-09 のユーザー実機確認(それまでは未共有で 0 と仮定していた)。
...
const golden: [string, Row, Row][] = [
  ...
  ["9.7", [76.7, 10.0, 1.1, 44.2, 132.0, 911375], [76.6, 12.0, 1.1, 44.2, 133.9, 918839]],
  ...
  ["9.12", [78.4, 4.5, 0.9, 44.0, 127.8, 1100085], [78.3, 3.8, 1.2, 43.9, 127.2, 1097187]],
  ["9.13", [77.0, 4.4, 0.5, 42.7, 124.6, 1137414], [77.0, 5.5, 0.4, 42.6, 125.5, 1141972]],
  ["9.14", [77.0, 4.7, 0.5, 42.7, 124.9, 1138934], [77.0, 5.3, 0.4, 42.6, 125.3, 1140959]],
  ...
  ["9.17", [77.0, 4.1, 0.5, 42.7, 124.3, 1128236], [77.0, 3.3, 0.4, 42.6, 123.3, 1123206]],
];
...
describe("表示スコアボーナスのゴールデンケース(2026-09-08 実機 20 ケース)", () => {
  ...
  it("20 ケースの実機との誤差が現在の範囲に収まる(悪化の検知。この値に合わせ込むためのテストではない)", () => {
    // 現状の最大誤差(2026-09-09、みこ・フワワの青ボード訂正後): アクティブ 0.1 / ボード 2.0 / パッシブ 1.6 /
    // SP 0.5 / 合計 1.9 / ユニットスコア 0.82%。上限はそれをわずかに上回る値で、下げる(精度を上げる)ときは
    // この上限も下げる。訂正前の上限(ボード 2.9 / 合計 2.7 / ユニットスコア 1.25%)から締め直した
    const limits = {
      active: 0.15,
      board: 2.1,
      passive: 1.7,
