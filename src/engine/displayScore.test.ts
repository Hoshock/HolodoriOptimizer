 ...
 *   スコアサポートだけの形成は全件一致する。発動率 UP 項の形は反証済みで未解決 — pending.md「SP 欄の発動率 UP 項」)
 ...
 * ボード / SP は Golden にしない（SP は発動率 UP 込みの残差 — pending.md「SP 欄の発動率 UP 項」）。K6 のパッシブ欄 2.9 は
...
describe("表示スコアボーナスのゴールデンケース(2026-09-08 実機 20 ケース)", () => {
  ...
  it("SP 欄は 20 ケース中 15 が完全一致し、外れるのは発動率 UP を含む形成だけ(±0.6)", () => {
    const mismatched = golden
      ...
      .map(([name]) => name);
    // 9.7 / 9.8 / 9.10 / 9.11 / 9.12 は SP に「スキル発動率 +X%」があり、その項の式が未解明(pending.md「SP 欄の発動率 UP 項」)
  ...
  it("20 ケースの実機との誤差が現在の範囲に収まる(悪化の検知。この値に合わせ込むためのテストではない)", () => {
    // 現状の最大誤差(2026-09-13 に「source 加算の総量 × projective 配分」へ載せ替えた後の回帰値):
    // アクティブ 0(完全一致) / ボード 0.5 / パッシブ 0.2 / SP 0.6 / 合計 0.5 / ユニットスコア 0.23%。
...
describe("赤スコアサポート +10 の 水着ノエル 0凸 入替(2026-09-11 実機観測。固定 0.88 × X を棄却した対照)", () => {
   ...
   * SP 欄は −1.3(ノエル 0凸の SP 100% × 12 秒。発動率 UP は 3期生 1 人で不発 — pending.md「SP 欄の発動率 UP 項」)。
