...
describe("表示スコアボーナスのゴールデンケース(2026-09-08 実機 20 ケース)", () => {
  ...
  it("20 ケースの実機との誤差が現在の範囲に収まる(悪化の検知。この値に合わせ込むためのテストではない)", () => {
    // 現状の最大誤差(2026-09-13 に SP 欄の発動率 UP を閉じた形へ替えた後の回帰値):
    // アクティブ 0(完全一致) / ボード 0.5 / パッシブ 0.2 / SP 0(完全一致) / 合計 0.5 / ユニットスコア 0.22%。
    // 載せ替え前は ボード 2.7 / パッシブ 1.8 / 合計 2.3 / ユニットスコア 0.99% だった。
