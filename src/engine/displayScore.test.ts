 ...
 * 実機値へ訂正した後の数字。訂正でボード欄・合計・ユニットスコアの最大誤差はいずれも小さくなった):
 * - アクティブ欄: ±0.1(5 人共通タイムライン・確率 55/46/37)
 ...
 * - 合計: −1.4〜+1.8 pt、ユニットスコアは −0.6%〜+0.8%
...
const golden: [string, Row, Row][] = [
  ...
  ["9.7", [76.7, 10.0, 1.1, 44.2, 132.0, 911375], [76.6, 11.9, 1.1, 44.2, 133.8, 918446]],
  ["9.8", [63.7, 13.9, 3.0, 38.6, 119.2, 1106433], [63.7, 13.2, 1.9, 39.1, 117.9, 1099871]],
  ...
  ["9.11", [75.6, 12.5, 2.4, 45.2, 135.7, 1191293], [75.5, 12.6, 1.9, 44.7, 134.7, 1186239]],
  ["9.12", [78.4, 4.5, 0.9, 44.0, 127.8, 1100085], [78.3, 3.8, 1.3, 43.9, 127.3, 1097670]],
  ["9.13", [77.0, 4.4, 0.5, 42.7, 124.6, 1137414], [77.0, 5.4, 0.4, 42.6, 125.4, 1141466]],
  ["9.14", [77.0, 4.7, 0.5, 42.7, 124.9, 1138934], [77.0, 5.2, 0.4, 42.6, 125.2, 1140453]],
  ["9.15", [77.0, 4.6, 0.5, 42.7, 124.8, 1138427], [77.0, 4.7, 0.4, 42.6, 124.7, 1137921]],
  ["9.16", [77.0, 4.3, 0.5, 42.7, 124.5, 1131410], [77.0, 4.1, 0.4, 42.6, 124.1, 1129394]],
  ["9.17", [77.0, 4.1, 0.5, 42.7, 124.3, 1128236], [77.0, 3.2, 0.4, 42.6, 123.2, 1122703]],
];
...
describe("表示スコアボーナスのゴールデンケース(2026-09-08 実機 20 ケース)", () => {
  ...
  it("20 ケースの実機との誤差が現在の範囲に収まる(悪化の検知。この値に合わせ込むためのテストではない)", () => {
    // 現状の最大誤差(2026-09-09、みこ 36 / 0・フワワ 45 / 0 の青ボード訂正後): アクティブ 0.1 / ボード 2.0 /
    // パッシブ 1.6 / SP 0.5 / 合計 1.8 / ユニットスコア 0.78%。上限はそれをわずかに上回る値で、
    // 下げる(精度を上げる)ときはこの上限も下げる。訂正前の上限(ボード 2.9 / 合計 2.7 / ユニットスコア 1.25%)から締め直した
    const limits = {
      active: 0.15,
      board: 2.1,
      passive: 1.7,
