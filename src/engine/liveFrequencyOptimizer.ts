 ...
 * `removedNodeIds`（現在 ON の頻度マスを外す）。**コスト（`additionalNodeCount`）は追加ぶんだけを数える** —
 * 省素材案や tie-break の「追加解放数」は「今後新しく開ける必要があるマス数」という意味を保つ（2026-09-10）。
 * 外すと素材は実際に返ってくる（2026-09-16 ユーザー報告）が、返還ぶんを差し引かないので、この値は
 * **これから払う素材の上限**になる（返還の単位が未確認なので、有利な側へ丸めない）
 */
