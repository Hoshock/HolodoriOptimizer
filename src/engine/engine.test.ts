...
describe("optimize", () => {
  ...
  /**
   * 衣装スキル発動・パッシブ全員発動の「しぼりこみ」は 2026-09-16 に撤去した(ユーザー指示
   * 「衣装スキル発動とパッシブのボタンは削除する。内部的にはそれらを考慮せず全探索する」)。
   * 不発の編成も候補に残ることを固定して、条件つきの枝刈りが復活していないことを見る
   */
  it("衣装スキル・パッシブが不発の編成も候補から外さない", () => {
    // 衣装: leader の条件は cute 2 枚。満たさない編成も候補に残る
    const all = optimize({ leader, topN: 100 }, pool, holomenMap);
    expect(all.candidates.some((c) => !c.breakdown.costumeSkillActive)).toBe(true);

    // パッシブ: gamers 2 人(h5 / h6)が条件。相方のいない編成も候補に残る
    ...
    const result = optimize({ leader, topN: 100 }, withCond, holomenMap);
    expect(
      result.candidates.some(
        (c) =>
          c.members.some((m) => m.id === "cond-passive") &&
          !c.members.some((m) => m.holomenId === "h6"),
      ),
    ).toBe(true);
  });
