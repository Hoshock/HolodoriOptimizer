 ...
 *   リーダーの衣装・赤ボードのスコアサポートは常時なので静的に (1 + X/100) 倍。
 *   この差分による配賦はサーバー側のカテゴリ別算出の再現ではない(上の外部情報)。次の逆解析の対象は
 *   スキルツリー(ホロメンボード)欄とパッシブ欄の算出式 — pending 12。
 * 【実機で否定(2026-09-11)】赤「全員のスコアサポート効果 +X%」の増分を**すべてボード欄へ寄せ、パッシブ欄は赤で変わらない**
 *   という上の配賦は実機と一致しない。同じ 5 人・同じリーダーで赤の歌唱者条件を OFF → ON にして X を 28.1 → 52.1 にすると、
 *   実機は ボード 38.2 → 59.1(+20.9)・パッシブ 3.1 → 3.3(+0.2)・合計 164.3 → 185.4(+21.1)で、パッシブ欄も動く
 *   (2026-09-08 の 0 → 28.1 でも ボード +23.5・パッシブ +0.9・合計 +24.4)。合計の増分も現行の (1 + X/100) 倍
 *   (基準 ≈ 92.4 で +26.0 / +22.2)より小さく、X 1 ポイントあたり 0.87〜0.88(2 回の観測でほぼ同じ)。基準が何か
 *   (アクティブ候補のいる秒の割合 0.875 などが候補)と、ボード欄 / パッシブ欄の分け方(パッシブ側の増分は X に対して
 *   頭打ち)は 2 点の差分だけでは決まらないので**式は変えず**、実機値をゴールデン(displayScore.test.ts「赤の歌唱者条件と
 *   黄の 4 象限」)と status.md に固定した。ケース別の定数で合わせることはしない(pending 12)。
 ...
 * 実機では合計は近いのに配賦がずれる(ケース A: 実機 ボード 36.7 / パッシブ 3.1 に対しモデル 39.6 / 1.8 —
 * 赤の「全員のスコアサポート」の増分をボード欄に寄せすぎている)。サーバー側はカテゴリごとに独立した値を返して
 * いるので【外部情報】、この差分による配賦は**ゲーム内部の式ではなく、算出式が不明なあいだの近似**である。
 * 2026-09-11 に「赤のスコアサポートだけが違う 2 点」(歌唱者条件 OFF / ON。X = 28.1 / 52.1)が出て、
 * (a) パッシブ欄も赤で動く(3.1 → 3.3)、(b) 合計の増分は X × 0.88 程度で (1 + X/100) 倍(× 0.924)より小さい、
 * ことが分かった。ただし基準の正体と配賦の形は 2 点では決まらない(候補はファイル冒頭)ので式はまだ変えない — pending 12
 */
