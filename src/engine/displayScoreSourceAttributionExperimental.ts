 ...
 * - 青 1 人・発動率のみ K7（2026-09-13）: 同じ総量式が通る。青ボードの % が 1 人ぶんしかないので、配賦側の重み
 *   `W_blue` を単独で拘束できる（`displayScoreProjectiveWeightExperimental.ts`）。そのため環境はメンバーごとの
 *   青 % （`blueRatePercent` / `blueFrequencyPercent`）も持つ — `pBlue` は上限 1 で頭打ちするので元の % は別に要る。
 *
