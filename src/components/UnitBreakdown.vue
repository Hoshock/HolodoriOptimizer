    ...
    <div class="footnotes">
      ...
      <p v-if="detailOpen">
        ...
        <span
          ...
          2凸の +10%
          から割り戻した推定で、合計で数点の誤差があります。「開花状況を考慮する」「ボード状況を考慮する」を外したとき（全カードからさがすときも）は、開花最大・ホロメンボード全解放の状態として試算します。ボードは色ごと（赤・青・黄・緑）に外せて、外した色だけが全解放になります。ホロメンボードはマスの表記値の合計に、コネクトマスに入れた範囲と倍率の増幅を加えて試算します。赤ボードはリーダーのホロメンのものだけが効き、固定値はメンバー各自に、割合は
          5
