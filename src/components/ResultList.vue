  ...
  >
    <template #page="{ item: candidate, index: rank }">
      <div class="result-card">
        ...
        <!--
          お気に入りの星は結果の 1 件（スコアの数字があるパネル）の右上、右端のアイコン列の真上に置く
          （2026-09-09 ユーザー指定。結果パネルの外・その内側の右角・パネルの角に重ねる案は差し戻された）。
          大きさは順位の円と同じ 28px
        -->
        <button
          type="button"
...
<style scoped>
/*
 * 1 件のパネルと、その右上に置くお気に入りの星。
 * 星は右端のアイコン列(リーダー行の衣装アイコン・メンバー行の開花アイコン)の真上、
 * 見出しの行(順位の円とスコア)と同じ高さに置く(2026-09-09 ユーザー指定「衣装アイコンの列の上に」。
 * 角に重ねる案・パネルの外へはみ出す案はどちらも差し戻された)。
 * 位置は 390px の実測で合わせた(星の中心とアイコン列の中心・見出しの行の中心が一致する)
 */
.result-card {
  position: relative;
}
...
.result-head {
  ...
  display: flex;
  gap: 8px;
  /* 右上の星（36px の当たり判定）の下に文字が潜らないぶんの余白 */
  padding-right: 36px;
}
