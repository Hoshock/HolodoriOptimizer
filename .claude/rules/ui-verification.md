...
- **検証で触ったオプションは保存されるので、続けて別の検証をする前に消す**（除外が `selection`、ほかが `search-all` / `search-options` に残る — `.claude/rules/storage-compat.md`）。枠の選択（リーダー・メンバー・曲）は 2026-09-16 から保存しないので `reload` で消えるが、除外とオプションは残る。検証の前後で `localStorage.removeItem("holodori-optimizer:selection")` のように消すか、`close` → `open` で開き直す（開き直すと localStorage ごと消える）。
