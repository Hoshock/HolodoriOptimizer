...
- **検証で触った入力は保存されるので、続けて別の検証をする前に消す**（2026-09-14 から、さがすときの枠の選択・除外・オプションが `selection` / `search-all` に残る — `.claude/rules/storage-compat.md`）。`eval` の要素選択を間違えてメイン画面のメンバー枠を押すと、そのカードが固定されたまま次の探索の結果が変わる（2026-09-15 に実際に汚れた）。検証の前後で `localStorage.removeItem("holodori-optimizer:selection")` のように消すか、`close` → `open` で開き直す（開き直すと localStorage ごと消える）。
