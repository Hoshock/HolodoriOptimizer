...
- UI の入口を外しても保存済みの状態は消さない（除外するカードは 2026-09-08 に入口を外したが `excludedIds` と探索への反映は保持 — ユーザー指示「内部的には保持しつつ UI からは削除」）。設定キーの既定値を変えるときは保存済みの値を尊重する（`search-all` の既定を「持っているカードのみ」に変えたが保存値はそのまま読む）。
