...
- **1 つの source だけを変えた matched pair では、その source の native カテゴリ（リーダー衣装 → 衣装、赤 → ボード、パッシブ → パッシブ）以外の 2 欄が共通倍率で scale する**（= 非 native 2 欄の比が保存される）。表示 0.1 の量子化込みで、2 欄とも情報を持つ 12 件（リーダー 4・赤 8）に反例なし。source から native を選ぶのは可、ケース ID で式を分けるのは不可。`C = λW_C / B = λW_B / P = λW_P` という内部実装は仮説にとどめ、raw weight の式は未解明。
