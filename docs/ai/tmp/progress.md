...
| 期待値最適化 | Step 1〜3+UI 刷新+開花対応+ピッカー操作性+SEO・改名+保存層+おかゆモード+しぼりこみ+曲ピッカー刷新+部品統一+計算仕様の検証・青ボード入力・Step 0 まで完了・デプロイ済み（main = a8df0eb） | ユニット（お気に入り編成）の仕様待ち、緑・所属ボーナスの置き場の相談、Step 4 はユーザーの再開指示待ち |
