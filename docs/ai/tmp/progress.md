...
| 期待値最適化 | Step 1〜3+UI 刷新+開花対応+ピッカー操作性+SEO・改名+保存層+おかゆモード+しぼりこみ+曲ピッカー刷新+部品統一+計算仕様の検証・青ボード入力・Step 0 まで完了・デプロイ済み（main = 9336c72） | ユニット（お気に入り編成）の仕様待ち、緑・所属ボーナスの置き場の相談、Step 4 はユーザーの再開指示待ち |
...
## コンパクション地点のログ（2026-09-06 housekeep 13 回目）

- 未コミットの変更: 棚卸し 13 回目の反映のみ — `.claude/rules/ui-design.md`（Step 0 アカウント・実機の形の再現・記号 1 文字・値の表示のみ・並び順の保持・連打ズーム・スクロールロックの備考を統合）、`.claude/rules/ui-verification.md`（dev サーバ・ブラウザセッションの再起動手順を現状に）、docs/ai/tmp/（rules.md 空化・plan.md 完了サマリと残作業・pending 4/7・progress.md 要約）。src/ は main（9336c72）と一致
- 未 push: 同上
- 次のアクション: plan.md「残作業」— ユニット（お気に入り編成）の仕様はユーザーから、緑ボード・所属ボーナスの入力の置き場を相談（pending 4）、Step 4 はユーザーの再開指示待ち。余白バグ（pending 9）と開花の実文言（pending 7）はユーザーの実機確認を待つ。新カードを追加するときは `reading` も必ず入れる
- 参照すべき方針: 分担表= CLAUDE.md、UI 制約= `.claude/rules/ui-design.md`、UI 確認手順と実行環境の備考= `.claude/rules/ui-verification.md`、保存データの互換= `.claude/rules/storage-compat.md`、規約= `.claude/skills/` の claude-md/rules/skills-convention、計算仕様（変わらない事実）= parameter-calculation スキル（青ボードの構成は references/blue-board.md）、アカウントの現在値・実測カード= `docs/ai/tmp/status.md`（棚卸し対象外）、権利= docs/human/rights-policy.md、ゲーム実仕様= docs/human/game-spec.md、期待値の仮定値= src/data/live.ts（pending 6）、開花= src/data/bloom.ts（2凸 1.10 は確定、スキル ×1.1 は仮定）、青ボード= src/data/blueBoard.ts + src/storage/boards.ts + BoardSheet.vue + HolomenPicker.vue、カードの解決= src/data/resolve.ts（開花 → ボードの 1 本）、おかゆモード= src/composables/useOkayuMode.ts、読みの比較= labels.ts の `readingSortKey`
- 運用メモ: 機能変更は branch `claude/holodor-optimizer-party-ahh84e` と main の両方へ push し、デプロイは deploy.yml の完了を Monitor で確認。UI 変更はサンプル画像を共有して承認を得てから push（承認前は stash）。実測手順と実行環境の備考は ui-verification.md
