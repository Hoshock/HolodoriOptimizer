...
- **2026-09-06（パラメータ計算仕様の整理）**: ユーザーがカード詳細 P/T/S の計算仕様（青・緑ボード反映、赤・メモリー・強化ボーナス・スキル不反映、★5 2凸 = +10%、所属ボーナス上限 +900、5% は切り上げ）と実機の現在値（メモリー 5.8%、強化ボーナス 2.73%、緑ボード一覧、おかゆ・ころね・マリンの青ボード、実測カード 4 枚）を共有 → 変わらない事実は知識スキル `parameter-calculation`、時点情報は `docs/ai/tmp/status.md`（常設・棚卸し対象外。housekeep スキル・CLAUDE.md・docs/index.md に反映）。game-spec.md の育成・ボード節と pending 4・7 を更新。エンジンの仮定値（bloom.ts 1.05）は未変更
