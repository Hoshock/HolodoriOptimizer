...
- **2026-09-01（バッジのアイコン化）**: スキル・役割の枠つきバッジを廃止し、正円(26px 細線)のアイコンに統一（衣装=Tシャツ / SP・A・P=文字アイコン / リーダー=王冠 / 固定=ピン、自作 SVG・`SkillIcon.vue`）。テキストラベルは aria-label へ。本文の固定高をやめ行コンテナの中央揃えで 1 行/2 行の実高の真ん中に自動整列。結果一覧のリーダー行常時表示も同日実施。サンプル画像 3 往復で確定、390px 実測済み。
- **2026-09-01（文言刷新+リーダー常時おまかせ）**: 見出しの括弧注記を全廃し、見出しは名詞ラベル（さがす対象/除外するカード/リーダー/メンバー/曲/さがす）、件数は行ボタン内の右寄せ値（カードを選ぶ … n枚）、任意スロットは既定値「おまかせ」を実値表示に。リーダーは全カードでも常におまかせ可（canRun はプール空のみ false、ヒント文 2 つ削除）。エンジンは組合せ 1 回列挙 × 衣装スキル同型クラス評価+最大倍率の上限枝刈りに組み替え（`optimize.ts`）— 全カード・リーダーおまかせ 791,635,460 通りが約 4 秒で完走（初回のみ dev 変換で 20 秒台）。健全性テスト（クラス評価=各リーダー固定の最良と一致）を追加し 30 件 green。
- **2026-09-01（Step 3 曲別最適化）**: ステップ 5「曲を選ぶ（指定しなくてもOK）」を追加（さがすは 6 へ）。固定高の曲スロット+検索つき SongPicker（181 曲、m:ss・EXPERT Lv 表示、選択中は太枠）。worker が songId → 曲長を解決し期待値へ反映（未指定は代表値 128 秒）。390px 実測で代表 579,524 → 100 秒曲 608,875 と曲依存を確認。
- **2026-09-01（詳細モーダルの内訳改善）**: 内訳表を絶対値主体（% は括弧の補足）にして 3 行の和が見出しの総合期待スコアと一致するよう丸めを SP 行に寄せ、パラメータ表を h4「ユニットスコア」配下へ移して合計行を追加（右下=ユニットスコアで環が閉じる）、前段から変化していないセルは淡色化。390px 実測+検算一致を機械確認。
- **2026-09-01（Step 2 期待値エンジン）**: アクティブ・SP の期待寄与を導入。仮定値（発動率 低0.25/中0.5/高0.75、SP 1 回/曲、スコアサポート重み 1、代表曲長=中央値 128 秒）は `src/data/live.ts` に隔離（pending 6）。モデルは `src/engine/live.ts`、探索はカード単位前計算+liveSum 加減算で速度劣化なし（11.3M 通り完走確認）。結果一覧・詳細は総合期待スコア表示+内訳 3 行（ユニット/アクティブ/SP）、旧「試算に含まれない」注記を削除し末尾免責に仮定の旨を追記。テスト 29 件 green。
- **2026-09-01（Step 1 改修 2）**: 全カード選択中は「カードを選ぶ」ボタン自体を disabled に（開けるのに何もできないモーダルを廃止）。メンバー枠は先頭の空き枠のみ有効（以降は disabled、解除時は上へ詰める）。390px 実測で入力順制約・詰め・モード切替を確認。
- **2026-09-01（Step 1 改修）**: 「カードを選ぶ」（動作）と「全カード」（トグル）の同形並置が分かりにくいとの指摘を受け、探索範囲をセグメンテッドコントロール「全カード | 持っているカード」（両状態が見える）+ 全幅「カードを選ぶ」ボタンに分離。390px 実測で切替・保存・run 可否を確認。
- **2026-09-01（Step 1 タブ統合）**: 「全カード / 持っているカード」タブを廃止し OptimizerPanel を単一化。ステップ 1 所持（+「全カード」トグルボタン、localStorage 保存）/ 2 除外（所持と独立） / 3 リーダー / 4 メンバー / 5 さがす。全カード ON 中は所持ピッカー全グレーアウト（登録リストは保持・表示）。エンジン・スコアは不変（除外リストの組み立てのみ）。390px 実測: 既定 ON・トグル永続化・所持 6 枚のおまかせ探索（36 通り）・全カード+除外 1 枚のリーダー固定探索（10,479,833 通り）完走、横スクロールなし。
