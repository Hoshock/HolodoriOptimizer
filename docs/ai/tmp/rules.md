...
（なし — 2026-09-11 の棚卸し 32 回目で 2 件を `ui-flow.md` へ例として統合: 「ユニット画面はゲームのユニット編成画面と同じ入力（曲なし）で出す」は Step 0 の項へ、「再実行中は前回の結果を薄く残して領域を空にしない」はスピナーの項へ。31 回目では 3 件を `ui-parts.md` / `ui-flow.md` / `ui-design.md` へ、30 回目では「スクショからの取り込み」の事例群を `ui-parts.md` / `ui-flow.md` / `structure-import` スキルへ統合済み）
