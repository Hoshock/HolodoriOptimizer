...
3. **スコアサポートのカテゴリ配賦 — 残るのは `W_blue` の式**
   ...
   - 探索済みで棄却された形: 「編成全体の評価値どうしの差」型（7 候補）と「編成に依存しない member-local」型（32 kernel × 2）。**残るのは「メンバー単位だが同時候補の競合を通じて編成に依存し、かつ青が 1 人・小さい % のときは正規化をほとんど受けない」形**。K1〜K4（R 15〜42）と K7（R 6）の食い違いは `W_blue` の `R` 線形性そのものを疑う根拠でもある。
   - 残差が集中する 水着フブキ 0凸 の行は、K7 で Active / Passive / 青が実機で確定したので**入力側の問題ではない**（2026-09-12 スナップショットの 15.0 / 0 の確からしさは 2 の課題として残る）。
