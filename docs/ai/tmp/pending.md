...
3. **スコアサポートのカテゴリ配賦 — 残るのは `W_blue` の式**
   ...
   - **「青持ちが複数いるから」でもない**（K2 と K3 は同じ青 3 人で残差が 58 倍違う）。診断で分かったのは 2 つ: (a)「青 × パッシブ支援の**交差項**」（自由係数なし）が K1 の残差を 0.04 以内で説明し、残差 0 の 4 行では厳密に 0 になる — 効くのは「パッシブ支援があるか」ではなく「その**対象が青を持つか**」、(b) K3 / K4 に残る +3.3 / +2.7 はメンバー単位に分解すると**発動率の大きいメンバー**（水着みこ 42 / 水着おかゆ 35.1）の取り分の過大で、水着フブキ の取り分ではない。加算型 `min(1, p0 + R/100)` の分母が膨らまない編成で過大になる形。`display-score.md`「4c」。
   - `A − B` 型（19 primitive kernel の全 342 通り）は 5 行すべての区間を満たすものが 0 件。member-local 型（64 通り）も 0 件。
