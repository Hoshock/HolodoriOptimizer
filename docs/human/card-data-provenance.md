...
「不明」と報告された区間はカードデータに何も足さない（記録がない状態がそのまま「不明」）。報告に含まれていた 恒常おかゆ 1〜5凸 Active と 水着おかゆ 3〜5凸 SP の「追加条件を括弧で連結する」表記は `cards.json` が最初からその表記なので変更していない（フォームを開いたブラウザ側が古い配信物だったとみられる）。
