...
- **記録のある variant は「未確認」にしない** — 実機観測・実機再構成に加えて抽出マスター（外部解析）由来もそのまま使う。外すと実機 SP 欄 20 編成の閉じた形（誤差 0）が崩れて実機と矛盾する（`src/engine/displayScoreSpecialColumn.test.ts`）。
...
2026-09-15 の第 2 弾で 16 枚を確認し、第 1 弾と合わせて **21 枚が「全区間が確認済 or 未確認」で確定**した。一覧は `src/data/bloomEvidence.ts` の `BLOOM_TEXT_VERIFIED_CARD_IDS`（`isBloomTextVerified()`）に持つ。**この一覧に載っていないカードは、カード効果の文言が入っていてもこの確認をまだ通していない**（取り込み元レコードのまま）— 開花文言フォームのピッカーは確認済みのカードを外すので、出てくるカードがそのまま「本確認がまだ」の集合になる。カード一覧（一覧のタイルとカード詳細）でも、確認がまだのカードはスキル文言を淡色で出す。
