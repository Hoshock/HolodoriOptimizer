...
| `unknown`                          | その段階の記録がない。文言は「不明」と表示し、計算には最近傍の凸（強化後の最大側レコード）の内容をそのまま流用する                                                              |
...
- `cardAtBloomWithProvenance()` は、最大レコードそのもの・確認済みvariant・再構成観測・抽出マスターvariant・不明を区別して返す。
...
## 2026-09-15 「不明 / 確認済」の2状態へ（推定の廃止）

ユーザー方針: **スキルの文言は「確認済」と「不明」だけで管理する。実機で確認が取れていない段階は、推定値を入れずに「不明」と書く。**

- 記録（`bloomVariants`）のない強化前の段階は `unknown` になり、カード詳細では**文言を「不明」と表示**する。
...
- **記録のある variant は「不明」にしない** — 実機観測・実機再構成に加えて抽出マスター（外部解析）由来もそのまま使う。外すと実機 SP 欄 20 編成の閉じた形（誤差 0）が崩れて実機と矛盾する（`src/engine/displayScoreSpecialColumn.test.ts`）。
- **テストは実機の値だけで固定する**（ユーザー方針「テストするときには必ず実機の値を使う。推定値でテストはしない」）。流用値から出した係数を数値で固定していた SP の「不明を含む編成」のテストは、実機どうしで言えること（アクティブ欄は一致するのに SP 欄は一致しない）だけに絞った。

### 反映した実機報告（開花文言フォーム 5 枚）

| カード                | 直したところ                                                                                                            |
| :-------------------- | :---------------------------------------------------------------------------------------------------------------------- |
| `sakura-miko-02`      | 衣装スキルのスコアサポート側にも「ピュアタイプ2人以上で」が明記される（`cardCorrections.ts` で訂正）。0凸 Active は不明 |
| `shirakami-fubuki-02` | 全区間そのまま（`rechecked`）                                                                                           |
| `ookami-mio-02`       | 0凸 Active は不明。ほかはそのまま（`rechecked`）                                                                        |
| `nekomata-okayu-01`   | 0〜3凸 Passive = キュートタイプ2人のテクニックが30%UP（実機観測の variant を追加）。0凸 Active は不明                   |
| `nekomata-okayu-02`   | 0〜2凸 SP・0凸 Active・0〜3凸 Passive はいずれも不明                                                                    |

「不明」と報告された区間はカードデータに何も足さない（記録がない状態がそのまま「不明」）。
