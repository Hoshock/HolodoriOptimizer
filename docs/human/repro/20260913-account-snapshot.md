...
ユーザーがサイトの「データの出力」でコピーした `holodori-optimizer/account` **version 1** をそのまま保存したもの（4 色ボードのマス ID・コネクトの配置・所持メンバーと開花・イベントメモリー・メンバー強化ボーナス）。**2026-09-13 時点の current snapshot**で、時点情報なので陳腐化を許容する。発動頻度 ownership 実験（F0〜F3）のあとにさらにボードが動いているので、**F3 の状態そのものではない**（下の「2026-09-12 からの差分」）。
...
| 鷹嶺ルイ                     |  23 |   0 |  20 |   4 | center=center-1/1400 leader=leader-3/1500 content=leader-2/2200 |
...
| フワワ・アビスガード         |  44 |  14 |   0 |   0 | center=center-1/1400 leader=leader-1/1500 card=content-3/2000   |
| モココ・アビスガード         |   0 |  28 |  20 |   4 | center=content-4/1150 card=card-3/2100 content=content-1/1100   |
| 音乃瀬奏                     |   0 |   0 |   0 |   4 |                                                                 |
| 一条莉々華                   |   0 |   0 |   0 |   4 |                                                                 |
| 儒烏風亭らでん               |   0 |   0 |   0 |   4 |                                                                 |
| 轟はじめ                     |   0 |   0 |   0 |   4 |                                                                 |
...
| フワワ・アビスガード | `fuwawa-abyssgard` |          12 |             0 |          24 |             0 | B-006×3 B-007×3 B-008×3                                                                                               |
...
| 鷹嶺ルイ             | `takane-lui`       |                   75 / 40 / 0 |                   86 / 62 / 0 |
...
| 対象                 | 変更                                                                                                                                                                                                                                 |
| :------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| フワワ・アビスガード | 赤 35 → 44（+R-009 / R-010 / R-019 / R-020 / R-022 / R-056 / R-058 / R-060 / R-061）、**青 0 → 14**（+B-001〜B-010 / B-016 / B-017 / B-023 / B-026）、**黄 16 → 0**（全マスが消えている）、コネクト content `leader-3`/1500 を外した |
...
| 鷹嶺ルイ             | 赤 0 → 23、コネクト center `content-4`/1150 → `center-1`/1400、leader に `leader-3`/1500 を追加、content `content-3`/1500 → `leader-2`/2200                                                                                          |
| モココ・アビスガード | 黄 10 → 20（+Y-014 / Y-015 / Y-016 / Y-023〜Y-029）                                                                                                                                                                                  |
| シオリ・ノヴェラ     | 緑 4 → 10（+G-006 / G-007 / G-008 / G-011 / G-018 / G-021）、コネクト center `center-4`/1150 を追加                                                                                                                                  |
| 古石ビジュー         | 緑 4 → 7（+G-006 / G-007 / G-008）、コネクト center `leader-2`/1650 を追加                                                                                                                                                           |
...
**フワワ・アビスガード のボードは実際に振り直されている（2026-09-13 ユーザー確認）。** この export ではじめて
青 14 マス（実効 発動率 24% / 発動頻度 0%）が現れ、同時に黄が 16 マスから 0 になっている。ユーザーが
**「シェアしたのが今のボードをちゃんと表してる」**と確認したので、「取り込みが青と黄を取り違えている」という
説明は落ちる — 出力は現在のボードを正しく写している。したがって**ホロメンボードは解放済みのマスを外して
振り直せる**。**外すと素材も返ってくる**（2026-09-16 ユーザー報告）。返還の単位（1 マスずつか・全額か）は未確認。

この青は**現在の値**であって、過去の観測へ遡及適用しない。発動頻度 ownership 系列（F0〜F3）と 2026-09-12 の
コーパスは、観測時点の出力が示していた「水着フワワ は青なし」をそのまま入力条件として使う
（`displayScoreCategoryCorpus.fixture.ts` の `frequencyTransferBlue()` が、この snapshot のフワワの青マスを
明示的に OFF にして観測時点を再構成する）。残るのは 2026-09-09 の実機報告（発動率 +45%）で、これは 0 とも 24 とも
違う第 3 の値なので、その時点のボード状態として別に扱う（`docs/ai/tmp/pending.md`
「2026-09-12表示スコアGoldenの入力条件再確認」）。

**発動頻度 ownership 実験について。** この日、さくらみこ と 猫又おかゆ のあいだで発動頻度マス 3 つ
（`B-013` / `B-020` / `B-031`）を 1 つずつ移す 4 状態を実機で観測した
（[display-score-20260913-frequency.md](./display-score-20260913-frequency.md)）。さくらみこ / 猫又おかゆ の青は
**実験の終了時点（F3 = さくらみこ 0 マス / 猫又おかゆ 3 マス）のまま**で、途中の F0〜F2 は transient なので
snapshot にはしない。ただし**この snapshot 全体は F3 時点そのものではない** — 実験のあとに フワワ・鷹嶺ルイ・
モココ・シオリ・古石ビジュー のボードとコネクトが動いているので、F 系列の入力はこの snapshot をそのまま使わず
`frequencyTransferBlue()` の再構成を通す。
