...
**フワワ・アビスガード の青は未確認（要再確認）。** この export ではじめて青 14 マス（実効 発動率 24% / 発動頻度 0%）が
現れ、同時に黄が 16 マスから 0 になっている。**いつ青が開いたのかは実機で確認できていない**ので、
この値を過去の観測へ遡及適用しない — 発動頻度 ownership 系列（F0〜F3）と 2026-09-12 のコーパスは、観測時に記録した
「水着フワワ は青なし」をそのまま入力条件として使い続ける（`displayScoreCategoryCorpus.fixture.ts` の
`frequencyTransferBlue()` が、この snapshot のフワワの青マスを明示的に OFF にして観測時点を再構成する）。
2026-09-09 の実機報告（発動率 +45%）とも一致しないので、フワワの青は 0 / 24 / 45 の 3 値が未決着のまま残る
（`docs/ai/tmp/pending.md`「2026-09-12表示スコアGoldenの入力条件再確認」）。
